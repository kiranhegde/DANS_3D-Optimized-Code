...
!*********************************************************************************************
 subroutine KwSSTdiffInterior(FacInx,NXX,NYY,NZZ,U,V,W,FirstEqDiff,SecndEqDiff)
 use MeshVar
 use MeanFlowVar
 use TurbulenceVar
 use KwSSTVariables
...
real(8)::NXX,NYY,NZZ,U,V,W,Mu_k,Mu_w,F1,F2,SigkMean,SigwMean,FirstEqDiff,SecndEqDiff
!*********************************************************************************************

Left = IDS(1,FacInx)
Rigt = IDS(2,FacInx)

SigkMean = 0.5*( Sigk(Left) + Sigk(Rigt) )
SigwMean = 0.5*( Sigw(Left) + Sigw(Rigt) )
